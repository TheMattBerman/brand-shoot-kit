# Brand Analysis

- Brand: Example Skin
- Product: Hydrating Face Serum
- URL: https://example.com/products/hydrating-face-serum
- Price tier: unknown
- Audience: daily routine shoppers
- Tone: brand-consistent ecommerce
- Palette: unknown
- Inferred product category: skincare

## Structured Scout Extraction
- Product name: Hydrating Face Serum Example Skin
- Brand name: Example
- Product type/category: skincare bottle / skincare
- Price (detected): unknown
- Variants/options: none detected
- Claims/benefits: Hydrating Face Serum Example Skin Clinical-grade hydration with a lightweight non-sticky finish
- Ingredients/materials/specs: Hydrating Face Serum Example Skin Hydrating serum with niacinamide and hyaluronic acid for daily barrier support
- Packaging text candidates: Hydrating Face Serum Example Skin, Example, Hydrating Face Serum
- Extraction warnings: low_confidence:price, low_confidence:variants, degraded_mode_source:html_heuristics_only, structured_source:none_detected_using_heuristics

## Product Preservation Brief
- Product type: skincare bottle
- Must preserve: bottle silhouette, cap geometry, front label hierarchy
- Can vary: camera angle (safe), background depth (minimal), neutral scene context
- Never change: brand name, ingredient claims, product count
- Distortion risks: small label text drift, cap deformation, glass glare
- Accuracy confidence: low

## Source Evidence Snapshot
- Source title: Hydrating Face Serum Example Skin
- Source H1: Hydrating Face Serum
- Source summary: Hydrating serum with niacinamide and hyaluronic acid for daily barrier support.
- Evidence keywords used in prompts: hyaluronic, niacinamide, glass, label
